...
static inline int s3c_gpio_do_setpull_pd(struct s3c_gpio_chip *chip,
				      unsigned int off, s3c_gpio_pull_t pull)
{
	return (chip->config->set_pull_pd)(chip, off, pull);
}
...
extern s3c_gpio_pull_t s3c_gpio_getpull_s3c24xx(struct s3c_gpio_chip *chip,
						unsigned int off);
extern int s3c_gpio_setpull_updown_pd(struct s3c_gpio_chip *chip,
			    unsigned int off, s3c_gpio_pull_t pull);
extern s3c_gpio_pull_t s3c_gpio_getpull_updown_pd(struct s3c_gpio_chip *chip,
					unsigned int off);
extern int s3c_gpio_setpin_updown_pd(struct s3c_gpio_chip *chip,
                 unsigned int off, s3c_gpio_pull_t level);
