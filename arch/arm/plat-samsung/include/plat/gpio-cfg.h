...
struct s3c_gpio_cfg {
	...
	int	 (*set_config)(struct s3c_gpio_chip *chip, unsigned offs,
			       unsigned config);
        s3c_gpio_pull_t	(*get_pull_pd)(struct s3c_gpio_chip *chip, unsigned offs);
	int		(*set_pull_pd)(struct s3c_gpio_chip *chip, unsigned offs,
				    s3c_gpio_pull_t pull);
 int             (*set_pin_pd)(struct s3c_gpio_chip *chip, unsigned offs,
                                    s3c_gpio_pull_t level);
};
...
extern int s3c_gpio_setpin_pd(unsigned int pin, s3c_gpio_pull_t level);
extern int s3c_gpio_setpull_pd(unsigned int pin, s3c_gpio_pull_t pull);
