...
int s3c_gpio_setpull_pd(unsigned int pin, s3c_gpio_pull_t pull)
/*
...
*/	
{
	...
	local_irq_save(flags);
	//ret = s3c_gpio_do_setpull(chip, offset, pull);
	ret = s3c_gpio_do_setpull_pd(chip, offset, pull);
	local_irq_restore(flags);

	return ret;
}
EXPORT_SYMBOL(s3c_gpio_setpull_pd);

int s3c_gpio_setpin_pd(unsigned int pin, s3c_gpio_pull_t level)
													/*level:00 output0
														 01	output1
														 10	input
														 11	previous status*/
{
        ...
        local_irq_save(flags);
       	ret = (chip->config->set_pin_pd)(chip, offset, level);
	 local_irq_restore(flags);

        return ret;
}

EXPORT_SYMBOL(s3c_gpio_setpin_pd);
...
int s3c_gpio_setpull_updown_pd(struct s3c_gpio_chip *chip,
			    unsigned int off, s3c_gpio_pull_t pull)
{
...
s3c_gpio_pull_t s3c_gpio_getpull_updown_pd(struct s3c_gpio_chip *chip,
					unsigned int off)
{
	void __iomem *reg = chip->base + 0x14;
	int shift = off * 2;
...
int s3c_gpio_setpin_updown_pd(struct s3c_gpio_chip *chip,
                 unsigned int off, s3c_gpio_pull_t level)
{
	void __iomem *reg = chip->base+0x10;
	unsigned int shift = (off & 7) * 2;
	u32 con;
