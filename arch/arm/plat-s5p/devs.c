...
#ifdef CONFIG_USB_ANDROID_RNDIS
/*diog.zhao 110116
if disable rndis function, num_functions is over than real function,
it will occur a wrong function pointer access.
Patch it.*/
...
struct platform_device s5p_device_recovery = {
	.name		= "s5pc110-recovery",
	.id		= -1,
};
EXPORT_SYMBOL(s5p_device_recovery);
