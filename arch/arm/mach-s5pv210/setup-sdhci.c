				    ...
				    struct mmc_card *card)
{
	...
	if(dev->id == 3) // wifi(3) and mmc uses different setting in c110
	{
	...
	if (ios->clock <= (600 * 1024)) {
		...
		else
		{
			...
			/* setting below value may make errors 
			ctrl3 = S3C_SDHCI_CTRL3_FCSELTX_BASIC |
...
#if defined(CONFIG_MACH_SMDKV210)  || defined(CONFIG_MACH_T34)
...
static int sdhci0_get_ext_cd(void)
{
       printk("====== %s =====\n",__FUNCTION__);
	return true;
}

static int sdhci1_get_ext_cd(void)
{
       printk("====== %s =====\n",__FUNCTION__);
	return true;
}

static int sdhci3_get_ext_cd(void)
{
	return true;
}
...
static struct s3c_sdhci_platdata hsmmc0_platdata = {
	...
	.get_ro         = sdhci0_get_ro,
       .get_ext_cd	=sdhci0_get_ext_cd,
#endif
};

static struct s3c_sdhci_platdata hsmmc1_platdata = {
	.get_ext_cd	=sdhci1_get_ext_cd,
};
