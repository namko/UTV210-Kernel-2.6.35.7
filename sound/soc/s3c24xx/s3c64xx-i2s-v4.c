...
#if 0//
#define pr_debug(x...) printk("[s3c64xx-i2s-v4]: "x)
...
void dump_i2s(struct s3c_i2sv2_info *i2s)
{
	printk(KERN_INFO "IISMOD=0x%x..IISCON=0x%x..IISPSR=0x%x\
			...
			readl(i2s->regs + S5P_IISAHB), i2s->regs);
...
void s5p_i2s_set_clk_enabled(struct snd_soc_dai *dai, bool state)
{
	...
	if (state) {
		...
		//clk_disable(i2s->iis_ipclk); //wang cut  i2s is not closed
		...
		struct snd_soc_dai *dai)
{
	...
	ur_debug("%s(),dai->id=%d, [%d]\n", __func__, dai->id, __LINE__);
	//dump_i2s(i2s); //wang add
...
static int s5p_i2s_resume(struct snd_soc_dai *dai)
{
	...
	ur_debug("%s(),        dai_active %d, IISMOD %08x, IISCON %08x\n", __func__,
			dai->active, i2s->suspend_iismod, i2s->suspend_iiscon);
	if (reg_saved_ok != true) {
		if (dai->id == 0) {
...
static __devinit int s3c64xx_iis_dev_probe(struct platform_device *pdev)
{
	...
	{
		i2s->iis_ipclk = clk_get(&pdev->dev, "i2s1");
		if (IS_ERR(i2s->iis_ipclk)) {
	...
	{
		i2s->iis_ipclk = clk_get(&pdev->dev, "i2s2");
		if (IS_ERR(i2s->iis_ipclk)) {
...
#if 1 //wang add
     clk_enable(i2s->iis_ipclk); //open i2s clk
#endif
if(pdev->id==0) 
{
