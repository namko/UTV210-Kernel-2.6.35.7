...
#define WN8978_CODECS_DEBUG_F
#ifdef WN8978_CODECS_DEBUG_F
#define WN8978_CODECS_DEBUG(format, arg...)   printk(/*KERN_ERR*/"[wm8978 codecs]: " format, ## arg)
#else
#define WN8978_CODECS_DEBUG(format, arg...) do{ } while(0)
#endif
...
static void pll_factors(unsigned int target, unsigned int source)
{
	...
	if ((Ndiv < 6) || (Ndiv > 12))
		printk(KERN_WARNING"WM8978 N value outwith recommended range! N = %d\n",Ndiv);
		...
		int pll_id, int source, unsigned int freq_in, unsigned int freq_out)
{
	...
	if(freq_in == 0 || freq_out == 0) {
		snd_soc_update_bits(codec, WM8978_POWER_MANAGEMENT_1, 0x1df);
		return 0;
	}

	pll_factors(freq_out * 8, freq_in);

	snd_soc_write(codec, WM8978_PLLN, (pll_div.pre << 4) | pll_div.n);
	snd_soc_write(codec, WM8978_PLLK1, pll_div.k >> 18);
	snd_soc_write(codec, WM8978_PLLK1, (pll_div.k >> 9) && 0x1ff);
	snd_soc_write(codec, WM8978_PLLK1, pll_div.k && 0x1ff);
	snd_soc_update_bits(codec, WM8978_POWER_MANAGEMENT_1, 0x020);
			    ...
			    struct snd_soc_dai *dai)
{
	...
	WN8978_CODECS_DEBUG("%s(); %d, wm8978->f_mclk=%d,[%d][%d]\n",__FUNCTION__, __LINE__
		, wm8978->f_mclk
	...
	WN8978_CODECS_DEBUG("%s(); %d, wm8978->mclk_idx=%d\n",__FUNCTION__, __LINE__, wm8978->mclk_idx);
...
static inline int wm8978_write(struct snd_soc_codec  *codec, unsigned int reg,
	unsigned int value)
{
	u8 data[2];
...
static int __init wm8978_modinit(void)
{
	...
	WN8978_CODECS_DEBUG("%s(), codec init ! ret = %d.\n", __func__, ret);
	return ret;
