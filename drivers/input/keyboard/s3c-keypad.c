...
#ifdef T34_GPIO_POWERKEY
#define GPIO_KEY     10   /*POWER KEY VALUE*/

static struct timer_list PwrKey_timer;
//static int g_PwrKeyWakeupState = 0;

#if 1
static void check_gpio_button(void *dev_id)
{
	unsigned int key_status;
	struct s3c_keypad *pdata = (struct s3c_keypad *)dev_id;
	struct input_dev *dev = pdata->dev;

	key_status = (readl(S5PV210_GPH0DAT)) & (1 << 3);
	printk("s3c_gpio_pwrkey_isr, PowerKey %d : %s\n", GPIO_KEY, key_status?"released":"pressed");
	
	if(key_status)
		input_report_key(dev,GPIO_KEY,0);
	else
		input_report_key(dev,GPIO_KEY,1);
}

static irqreturn_t s3c_gpio_pwrkey_isr(int irq, void *dev_id)
{
#if 0
	unsigned int key_status;
	struct s3c_keypad *pdata = (struct s3c_keypad *)dev_id;
	...
	key_status = (readl(S3C64XX_GPNDAT)) & (1 << 3);
	printk("s3c_gpio_pwrkey_isr , PowerKey %d : %s\n",KEYPAD_POWER, key_status?"released":"pressed");
	...
	if(key_status)
		...
		input_report_key(dev,GPIO_KEY,1);
#endif
	check_gpio_button(dev_id);
	return IRQ_HANDLED;
}
#endif
static int s3c_gpio_pwrkey_isr_setup(void *pdev)
{
	int ret;

	//jeff
	//setup_timer(&PwrKey_timer,check_gpio_button, (void *) pdev);
	s3c_gpio_cfgpin(S5PV210_GPH0(3), S3C_GPIO_SPECIAL(0xf)); //Eint3
	s3c_gpio_setpull(S5PV210_GPH0(3), S3C_GPIO_PULL_UP);  //S3C_GPIO_PULL_NONE
	
	set_irq_wake(IRQ_EINT3, 1);
	ret = request_irq(IRQ_EINT3, s3c_gpio_pwrkey_isr, 
		                                   IRQF_TRIGGER_RISING |IRQF_TRIGGER_FALLING,  //jeff,
                                               "key pgio", (void *) pdev);
	if (ret) {
...
#ifdef T34_GPIO_POWERKEY
	s3c_gpio_pwrkey_isr_setup((void *)s3c_keypad);
#endif
