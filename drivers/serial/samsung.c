...
EXPORT_SYMBOL(s3c24xx_serial_tx_empty); //davied changed this func for other file used
...
//--
	...
	baud = uart_get_baud_rate(port, termios, old, 0, 1843200*2);
...
//davied add
        if(port->line == 0)
        {
                printk(KERN_INFO"set baud = %d quot = %d /dev/s3c2410_serial%d baudclk_rate = %ld \n",baud,quot,port->line,ourport->baudclk_rate);
        }
//--
...
#if defined(CONFIG_UART0_3M)
//davied add for bt sleep
struct clk * s3c24xx_serial_getsrcclk(int index)
{
        struct clk *sclk = NULL;
        struct s3c24xx_uart_port *ourport;
        ourport = &s3c24xx_serial_ports[index];
        sclk = clk_get(&ourport->port.dev,"sclk_uart");
        return sclk;
}
EXPORT_SYMBOL(s3c24xx_serial_getsrcclk);
struct uart_port * s3c24xx_serial_get_port(int index)
{
        struct uart_port * tmp_port;
        struct s3c24xx_uart_port *ourport;
        ourport = &s3c24xx_serial_ports[index];
        tmp_port = &ourport->port;
        return tmp_port;
}
EXPORT_SYMBOL(s3c24xx_serial_get_port);
//--
#endif
...
#if defined(CONFIG_UART0_3M)
	if (!port->line) {
		src0_12 = __raw_readl(S5P_CLK_SRC_MASK0)&(1<<12);
		ip3_17 = __raw_readl(S5P_CLKGATE_IP3)&(1<<17);
	}
#endif
...
#if defined(CONFIG_UART0_3M)
		if (!port->line) {
			__raw_writel(__raw_readl(S5P_CLK_SRC_MASK0)|src0_12, S5P_CLK_SRC_MASK0);
			__raw_writel(__raw_readl(S5P_CLKGATE_IP3)|ip3_17, S5P_CLKGATE_IP3);
		}
#endif
		clk_enable(ourport->clk);
...
static int s3c24xx_serial_init_ports(struct s3c24xx_uart_info **info)
{
	...
	{
#if defined(CONFIG_UART0_3M)
		if(i == 0)/*davied add for escape uart 0~1 */
                        continue;
#endif
