...
)
{
	...
	if (country_offset != 0) {
		...
		//Check the current connection and channel
		dev_wlc_ioctl(dev, WLC_GET_CHANNEL, &ci, sizeof(ci));
		channel = dtoh32(ci.target_channel);
		if ( (channel >11 && !strcmp(country_code,"US")) || (channel == 14 && strcmp(country_code, "JP")))
		{
			printf("!!!Set country in wrong range. Reset it!!!\n");
			dev_wlc_ioctl(dev, WLC_GET_SSID, &ssid, sizeof(ssid));
			if (ssid.SSID_len >0)
			{
				printf("!!!Disconnect from current.!!!\n");
				(void)dev_wlc_ioctl(dev, WLC_DISASSOC, &scbval, sizeof(scb_val_t));
				OSL_DELAY(10000);
			}
			ci.target_channel = htod32(1);
			dev_wlc_ioctl(dev, WLC_SET_CHANNEL, &ci.target_channel, sizeof(int));
		}
