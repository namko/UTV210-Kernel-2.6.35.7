...
static void wlan_proc_entry_chip(void)
{
    create_proc_read_entry("wlan_chip", S_IRUGO, NULL, wlan_chip_read_proc, NULL);
}

static void wlan_proc_entry_chip_remove(void)
{
...
{
	if (wifi_irqres) {
		printk("wifi_get_irq_number   wifi_irqres=%x\n",wifi_irqres->start);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35)
    cnt = netdev_hw_addr_list_count(&(dev->mc));
#else
 	mclist = dev->mc_list;
 	cnt = dev->mc_count;
#endif
