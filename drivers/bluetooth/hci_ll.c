...
static void samsung_serial_fifo_on() {
	struct uart_port * port = s3c24xx_serial_get_port(0);
	unsigned int ufcon = read_regl(port, S3C2410_UFCON);
	ufcon |= S3C2410_UFCON_FIFOMODE;
        write_regl(port, S3C2410_UFCON, ufcon);
     //   printk(KERN_INFO "%s ####!!!!!!!!!!!!!!!!!!!!!!!!!## dev = ttyHS%d ufcon = %x \n",__FUNCTION__,port->line,ufcon);
}

static void samsung_serial_fifo_off() {
	struct uart_port * port = s3c24xx_serial_get_port(0);
        unsigned int ufcon = read_regl(port, S3C2410_UFCON);
        ufcon &= ~S3C2410_UFCON_FIFOMODE;
        write_regl(port, S3C2410_UFCON, ufcon);
       // printk(KERN_INFO "%s ##!!!!!!!!!!!!!!!!!1#### dev = ttyHS%d ufcon = %x \n",__FUNCTION__,port->line,ufcon);
}
#else
static inline void samsung_serial_fifo_on() {}
static inline void samsung_serial_fifo_off() {}
#endif
//--
