...
#define EINT_AUTO_2_HDMI  
#ifdef EINT_AUTO_2_HDMI
static int first_enter_flg = 1;
#endif
...
irqreturn_t s5p_hpd_irq_handler(int irq, void *dev_id)
{
      ...
      if(!first_enter_flg)
	{
		...
		//read hdmi interrupt must after ... 
		HPDIFPRINTK("______ bdwg first enter !!\n");
		first_enter_flg = 0;
	}
	ret = irq_eint(irq);
	...
	if (atomic_read(&g_hdmi_status)) {
		/* HDMI on */
		ret = irq_hdmi(irq);
		HPDIFPRINTK("HDMI HPD interrupt  hdmi on\n");
	} else {
		/* HDMI off */
		ret = irq_eint(irq);
		HPDIFPRINTK("EINT HPD interrupt  hdmi off\n");
	}
#endif
	spin_unlock_irq(&g_hpd_struct.lock);
