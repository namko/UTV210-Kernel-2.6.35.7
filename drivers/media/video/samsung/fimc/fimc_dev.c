...
#if 0//by lifeijuan //urbetter -- for demo board
		if(ctrl->id ==0)
		{
			Device_PM_Control(PM_DEVICE_CAM_MAIN, PM_CONTROL_VAL_ON);
...
static int fimc_release(struct file *filp)
{
	...
	if(ctrl->id ==0 )
	{
	fimc_reset_cam_capture(ctrl);
	msleep(10);
	Device_PM_Control(PM_DEVICE_CAM_MAIN, PM_CONTROL_VAL_OFF);
	}
...
extern void s3c_fimc0_cfg_gpio_suspend(struct platform_device *pdev);
extern void s3c_fimc0_cfg_gpio_resume(struct platform_device *pdev);
...
int fimc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (atomic_read(&ctrl->in_use))
		fimc_clk_en(ctrl, false);
        s3c_fimc0_cfg_gpio_suspend(pdev);//
	return 0;
}
...
int fimc_resume(struct platform_device *pdev)
{
	...
	s3c_fimc0_cfg_gpio_resume(pdev);//yan
	ctrl = get_fimc_ctrl(id);
	pdata = to_fimc_plat(ctrl->dev);
