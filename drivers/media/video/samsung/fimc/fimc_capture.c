...
int fimc_reqbufs_capture(void *fh, struct v4l2_requestbuffers *b)
{
	...
	case V4L2_PIX_FMT_YUV422P:	/* fall through */
		...
		fimc_dbg("######%s: V4L2_PIX_FMT_YUV422P size=%d\n", __func__,size[0] );
		break;
	...
	case V4L2_PIX_FMT_NV61:
		...
		fimc_dbg("######%s: V4L2_PIX_FMT_NV61 size=%d\n", __func__,size[0] );
		break;
	case V4L2_PIX_FMT_NV12:
#if 0//lfj 0512,for macro use_fimc2_rec in SecCamera.h.If the macro not defined,address must be 2K align
		size[0] = cap->fmt.width * cap->fmt.height;
		size[1] = cap->fmt.width * cap->fmt.height/2;
#else/*if use_fimc2_rec is not defined*/
#if 1		
        size[0] = ALIGN(cap->fmt.width * cap->fmt.height, SZ_2K) +  ALIGN(cap->fmt.width * cap->fmt.height / 2, SZ_2K);
#else
        size[0] = ALIGN(cap->fmt.width * cap->fmt.height, SZ_2K);  
#endif   
	size[1] = ALIGN(cap->fmt.width * cap->fmt.height / 2, SZ_2K);
	size[3] = SZ_2K; /* Padding buffer */
	align = SZ_2K;
	fimc_dbg("######%s: V4L2_PIX_FMT_NV12 size_t[0]=%d  size_t[1]=%d   size_t[2]=%d\n", __func__,size[0],size[1],size[2] );
#endif
		break;
	case V4L2_PIX_FMT_NV21:
		size[0] = cap->fmt.width * cap->fmt.height;
		size[1] = cap->fmt.width * cap->fmt.height/2;
		size[3] = 16; /* Padding buffer */
		fimc_dbg("######%s: V4L2_PIX_FMT_NV21 size=%d\n", __func__,size[0] );
		break;
	case V4L2_PIX_FMT_NV12T:
		...
		align = SZ_8K;
		fimc_dbg("######%s: V4L2_PIX_FMT_NV12T size=%d\n", __func__,size[0] );
		break;
	...
	case V4L2_PIX_FMT_JPEG:
		...
		fimc_dbg("######%s: V4L2_PIX_FMT_JPEG size=%d\n", __func__,size[0] );
	default:
		break;
	}
