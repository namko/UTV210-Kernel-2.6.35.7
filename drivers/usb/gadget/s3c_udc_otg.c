...
/**
 *diog.zhao 110310
 *patch for the kernel panic while unregistering driver.
 */
	if (driver->unbind)
...
EXPORT_SYMBOL(s3c_udc_vbus_poweron);

int s3c_udc_vbus_poweroff()
{
	struct s3c_udc *dev = the_controller;
	int retval;
...
EXPORT_SYMBOL(s3c_udc_vbus_poweroff);
...
#ifdef CONFIG_PM
/*After probe function done,if there isnot any device is connected,
 *then power on otg and disable otg clock at first.
 */
