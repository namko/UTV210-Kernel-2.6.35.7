...
#if defined(CONFIG_MACH_SMDKC110) || defined(CONFIG_MACH_SMDKV210) || defined(CONFIG_MACH_T34)
...
void s3c_udc_cable_usbdetected(struct s3c_udc *dev)
{
    s3c_cable_check_status(2);
}

bool s3c_udc_is_adc_cable()
{
	int count = 0;
	mdelay(100);
	while (count < 10) {
	if ((readl(S3C_UDC_OTG_HPRT) & 0xc00) == 0xc00)/*diog.zhao@samsung.com,wrong adjudgement */
		count ++;
	else
		{
			count =0;
...
static irqreturn_t s3c_udc_irq(int irq, void *_dev)
{
...
#if defined (CONFIG_MACH_SMDKC110) || defined (CONFIG_MACH_SMDKV210) || defined(CONFIG_MACH_T34)
		printk(" s3c_udc_cable_usbdetected..\n");
		s3c_udc_cable_usbdetected(dev);
#endif
	...
	if (intr_status & INT_RESET) {
		...
		if ((usb_status & 0xc0000) == (0x3 << 18)) {
			...
			reset_available = 1;
#if defined (CONFIG_MACH_SMDKC110) || defined (CONFIG_MACH_SMDKV210) || defined(CONFIG_MACH_T34)
		printk(" s3c_udc_cable_disconnect..\n");
		s3c_udc_cable_disconnect(dev);
		if (dev->driver) {
			spin_unlock(&dev->lock);
			dev->driver->disconnect(&dev->gadget);
