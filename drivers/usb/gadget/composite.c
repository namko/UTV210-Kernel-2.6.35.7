...
/**
 * usb_remove_function() - add a function to a configuration
 * @config: the configuration
 * @function: the function being added
 * Context: single threaded during gadget setup
 *
 ...
 * zero for success else a negative errno value.
 */
int usb_remove_all(struct usb_configuration *config)
{
	struct usb_composite_dev	*cdev = config->cdev;
	...
	/* REVISIT *require* function->bind? */
	list_for_each_prev_safe(plist, temp, &config->functions)
	{
		function = container_of(plist, struct usb_function, list);
		if (function->unbind) {
			function->unbind(config, function);
		}
		dev_set_drvdata(function->dev, NULL);
		device_remove_file(function->dev, &dev_attr_enable);
		index = cdev->driver->function_count.counter;
 ...
 */
				if (f->upd_intNo)
					f->upd_intNo(f, intf->bInterfaceNumber);
			}
			dest += intf->bLength;
		}
