...
//Gaoping merge 20101215 if card is remove or read error ,donn't read any more
extern int mmc_blk_check_valid(struct block_device *bdev);
			  ...
			  struct buffer_head **bh, struct msdos_dir_entry **de)
{
	...
	int err, offset;
#if 1	//Gaoping merge 20101215 if card is remove or read error ,donn't read any more
	if(mmc_blk_check_valid(sb->s_bdev) != 0)		
		return -1;	
#endif
next:
	if (*bh)
